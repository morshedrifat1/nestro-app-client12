...
const PostCard = ({ post, refetch }) => {
  ...
  const handleVote = async (type) => {
    if(!user){
        return navigate(`/post-details/${post._id}`)
    }
  ...
  return (
    <div className="bg-boxbg border border-mainborder rounded-lg p-5">
      ...
      <p className="text-base leading-7 font-normal text-base-content mt-2">
        {post?.postDescription.split(" ").slice(0, 35).join(" ")}
        <span className="text-base-300 cursor-pointer">
          <Link to={`/post-details/${post._id}`}>
            {post?.postDescription.split(" ").length > 20 ? " ... See More": ""}
          </Link>
        </span>
      </p>
